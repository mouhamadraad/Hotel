...
//count nb of rooms and output variable to store room cards
  ...
  $room_res = select("SELECT * FROM `rooms` WHERE `adult`>=? AND `children`>=? AND `status`=? AND `removed`=? ORDER BY `id`", [$adults,$children,1, 0], 'iiii');
    ...
    while ($room_data = mysqli_fetch_assoc($room_res)) {
      ...
      if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
      {
        ...
        AND check_out > ? AND check_in < ? ";
          ...
          $tb = 0;
          if(($room_data['quantity']-$tb-$tb_fetch['total_bookings'])==0){
           continue;
          }

      }
      // Get facilities of the room with filters
    ...
    {$fac_row['name']}
       ...
       continue;
        ...
        // Generate the "Book now" button if the site is not in shutdown mode
        $book_btn = "";
        ...
        // Generate the room card HTML with headings for Features and Facilities
        $output .= "
